...
public class MergeSort {
    ...
    public static <T> T[] mergeSort(Class<T> type, T[] array, Comparator<T> comparator) {
        if (array.length == 0) {
    ...
    private static <T> T[] mergeSort(Class<T> type, T[] array, int start, int end, Comparator<T> comparator) {
        if (start == end) {
            @SuppressWarnings("unchecked")
        ...
        T[] right = mergeSort(type, array, (start + end) / 2 + 1, end, comparator);
        return merge(type, left, right, comparator);
    }

    public static <T> T[] merge(Class<T> type, T[] foo, T[] bar, Comparator<T> comparator) {
        @SuppressWarnings("unchecked")
