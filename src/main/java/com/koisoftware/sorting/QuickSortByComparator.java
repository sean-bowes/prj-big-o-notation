...
public class QuickSortByComparator {

    public static <T> T[] quickSort(T[] array, Comparator<T> comparator) {
        return quickSort(array, 0, array.length - 1, comparator);
    }

    private static <T> T[] quickSort(T[] array, int startIndex, int endIndex, Comparator<T> comparator) {
    ...
    public static <T> int partition(T[] array, int startIndex, int endIndex, Comparator<T> comparator, int pivotIndex) {
        if (pivotIndex == -1) {
            pivotIndex = getRandomPivotIndex(startIndex, endIndex);
        }

        ArrayUtils.swap(array, pivotIndex, endIndex);
        int swapIndex = startIndex - 1;
        for (int index = startIndex; index <= endIndex - 1; index++) {
            if (comparator.compare(array[index], array[endIndex]) <= 0) {
                swapIndex++;
                ArrayUtils.swap(array, swapIndex, index);
            }
        }

        ArrayUtils.swap(array, swapIndex + 1, endIndex);
        return swapIndex + 1;
    }
}
