...
public class BstLookup {
    public static <T> boolean lookup(TreeNode<T> rootNode, TreeNode<T> searchedNode, Comparator<T> comparator) {
        if (rootNode == null) {
            return false;
        }

        int compared = comparator.compare(rootNode.getValue(), searchedNode.getValue());
        if (compared < 0) {
