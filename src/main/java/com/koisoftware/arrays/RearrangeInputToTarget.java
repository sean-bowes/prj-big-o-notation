package com.koisoftware.arrays;
...
public class RearrangeInputToTarget {
    ...
    public static void rearrange(int[] input, int[] target) {
        ...
        int zeroIndex = ArrayUtils.findIndex(input, 0);
        int numMoves = 0;
        ...
        while (numMoves < input.length) {
            ...
            if (target[zeroIndex] == 0) {
                ...
                ArrayUtils.swap(input, zeroIndex, outOfOrderIndex);
                ...
                ArrayUtils.swap(input, zeroIndex, nextZeroIndex);
