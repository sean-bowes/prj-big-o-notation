...
import com.koisoftware.common.Pair;
...
public class NaryTreeSerialization {
    ...
    public static void serialize(NaryTreeNode<String> root, String filePath) throws IOException {
        if (filePath == null) {
            throw new NullPointerException("File path cannot be null.");
        }
        BufferedOutputStream fileHandle = new BufferedOutputStream(new FileOutputStream(filePath));
        try {
            if (root == null) {
            ...
            while (!queue.isEmpty()) {
                ...
                for (NaryTreeNode<String> child : children) {
                    if (child == null) {
                        throw new IllegalArgumentException("N-ary tree contains nulls.");
                    }
                    queue.add(child);
                }
                ...
                numNodes++;
                if (numNodes < 0) {
                    throw new IllegalArgumentException("Number of nodes cannot exceed size of maximum integer.");
                }
            }
    ...
    private static NaryTreeNode<String> deserialize(String filePath) throws IOException {
        if (filePath == null) {
            throw new NullPointerException("File path cannot be null.");
        }
        NaryTreeNode<String> root = null;
        BufferedInputStream fileHandle = new BufferedInputStream(new FileInputStream(filePath));
        try {
            int metadataSize = deserializeMetadataSize(fileHandle);
            if (metadataSize == 0) {
                return root;
            }
            List<Pair<Integer, Integer>> metadata = deserializeMetadata(fileHandle, metadataSize);
    ...
    private static void serializeMetadataSize(BufferedOutputStream fileHandle, int metadataSize) throws IOException {
        serializeInteger(fileHandle, metadataSize);
    }

    private static int deserializeMetadataSize(BufferedInputStream fileHandle) throws IOException {
        return deserializeInteger(fileHandle);
    }

    private static void serializeMetadata(BufferedOutputStream fileHandle, List<Pair<Integer, Integer>> metadata) throws IOException {
    ...
    private static List<Pair<Integer, Integer>> deserializeMetadata(BufferedInputStream fileHandle, int metadataSize) throws IOException {
    ...
    private static void serializeStrings(BufferedOutputStream fileHandle, List<String> strings) throws IOException {
        for (String s : strings) {
            serializeString(fileHandle, s);
        }
    }

    private static void serializeInteger(BufferedOutputStream fileHandle, int value) throws IOException {
    ...
    private static int deserializeInteger(BufferedInputStream fileHandle) throws IOException {
        int value = 0;
        for (int count = 0; count < 4; count++) {
    ...
    private static void serializeString(BufferedOutputStream fileHandle, String str) throws IOException {
        if (str == null) {
    ...
    private static String deserializeString(BufferedInputStream fileHandle, int length) throws IOException {
        if (length < 0) {
            throw new IllegalArgumentException(String.format("Length should be greater than zero. You passed: %d.", length));
        }
    ...
    private static void serializeChar(BufferedOutputStream fileHandle, char c) throws IOException {
        fileHandle.write((byte) (c & 0xFF));
        c >>>= 8;
        fileHandle.write((byte) (c & 0xFF));
    }

    private static char deserializeChar(BufferedInputStream fileHandle) throws IOException {
        char c = (char) 0;
